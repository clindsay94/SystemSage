...
DEFAULT_CONFIG: Dict[str, Any] = {
    "scan_options": {
        ...
        "excluded_paths": [
            ...
            "C:\\ProgramData\\Package Cache",  # Common noisy/system paths
        ],
        "scan_env_vars": True,
        ...
        "preferred_package_managers_linux": [
            "apt",
            "dnf",
            "yum",
            "pacman",
    ...
    "user_preferences": {"theme": "default", "always_show_details": False},
}
...
def _ensure_config_dir_exists():
    ...
    if not os.path.exists(CONFIG_DIR_PATH) and CONFIG_DIR_PATH != os.path.dirname(
        ...
        try:
            ...
            logger.error(
                f"Could not create configuration directory {CONFIG_DIR_PATH}: {e}"
            )
...
def load_config() -> Dict[str, Any]:
    ...
    if not os.path.exists(CONFIG_FILE_PATH):
        logger.warning(
            f"Configuration file not found at {CONFIG_FILE_PATH}. Creating with default settings."
        )
    ...
    try:
        ...
        logger.error(
            f"Error decoding JSON from {CONFIG_FILE_PATH}: {e}. Backing up and using default settings."
        )
        ...
        logger.error(
            f"Failed to load configuration from {CONFIG_FILE_PATH} due to an unexpected error: {e}. Using default settings.",
            exc_info=True,
        )
        return DEFAULT_CONFIG.copy()
...
def get_ignored_identifiers() -> List[str]:
    ...
    config = load_config()
    return config.get(
        "ignored_tools_identifiers", DEFAULT_CONFIG["ignored_tools_identifiers"]
    )
...
def remove_from_ignored_identifiers(identifier: str):
    ...
    config = load_config()
    if (
        "ignored_tools_identifiers" in config
        and identifier in config["ignored_tools_identifiers"]
    ):
...
if __name__ == "__main__":
    logging.basicConfig(
        level=logging.DEBUG
    )  # Set to DEBUG for detailed output from this module
