...
# { 'name': 'readable_name', 'detection_command': ['cmd', '--version'], 'list_co
mmand': ..., 'search_command_template': ..., 'update_command_template': ...}
...
PACKAGE_MANAGERS = {
    "winget": {
        ...
        "search_latest_cmd_template": ["winget", "search", "--exact", "{package_
name}"], # winget search --exact <package_id>
        "update_cmd_template": ["winget", "upgrade", "--exact", "{package_name}"
],
        "os": ["Windows"]
    },
    "choco": {
        ...
        "search_latest_cmd_template": ["choco", "search", "{package_name}", "--e
xact"], # choco search <package_id> --exact
    ...
    "scoop": {
        ...
        "search_latest_cmd_template": ["scoop", "search", "{package_name}"], # s
coop search <package_name> (exactness varies)
    ...
    "brew": {
        ...
        "search_latest_cmd_template": ["brew", "info", "{package_name}"], # brew
 info <formula>
    ...
    "apt": {
        "name": "APT",
        "detection_exe": "apt-get", # apt itself is often a script, apt-get is m
ore fundamental
        "version_args": ["--version"],
 ...
 # apt-cache policy <package_name> or apt show <package_name>
        "update_cmd_template": ["sudo", "apt", "install", "--only-upgrade", "{pa
ckage_name}"], # Or apt upgrade {package_name}
        "os": ["Linux"]
    },
    "dnf": {
        "name": "DNF",
        ...
        "search_latest_cmd_template": ["dnf", "info", "{package_name}"], # dnf i
nfo <package_name>
    ...
    "yum": {
        ...
        "search_latest_cmd_template": ["yum", "info", "{package_name}"], # yum i
nfo <package_name>
...
,
    ...
    "pacman": {
        ...
        "search_latest_cmd_template": ["pacman", "-Si", "{package_name}"], # pac
man -Si <package_name> (sync DB info)
        "update_cmd_template": ["sudo", "pacman", "-Syu", "{package_name}"], # T
his updates all then installs, or just -S for specific
        "os": ["Linux"]
    },
    "snap": {
        "name": "Snap",
        ...
        "search_latest_cmd_template": ["snap", "info", "{package_name}"], # snap
 info <snap_name>
    ...
    "flatpak": {
        ...
        "search_latest_cmd_template": ["flatpak", "remote-info", "--user", "flat
hub", "{package_id}"], # Assumes flathub, package_id is like org.example.App
        "update_cmd_template": ["flatpak", "update", "-y", "--user", "{package_i
d}"], # For user installation
        "os": ["Linux"]
    }
}
...
TOOL_TO_PM_PACKAGE_MAP = {
    "python": {"apt": "python3", "brew": "python3", "choco": "python3", "winget"
: "Python.Python.3"},
    "pip": {"apt": "python3-pip", "brew": "python3", "choco": "python3"}, # pip
comes with python
    "java": { # This is tricky due to JDK/JRE and vendors
        "apt": "openjdk-17-jdk", "brew": "openjdk@17", "choco": "openjdk",
        "winget": "Oracle.JavaRuntimeEnvironment" # Example, many Javas in winge
t
    },
    "node": {"apt": "nodejs", "brew": "node", "choco": "nodejs", "winget": "Open
JS.NodeJS"},
...
"winget": "Docker.DockerDesktop"},
    "kubectl": {"apt": "kubectl", "brew": "kubernetes-cli", "choco": "kubernetes
-cli", "winget": "Kubernetes.kubectl"},
    "vscode": {"snap": "code", "brew": "visual-studio-code", "choco": "vscode",
"winget": "Microsoft.VisualStudioCode"},
    "maven": {"apt": "maven", "brew": "maven", "choco": "maven", "winget": "Apac
he.Maven"},
    "gradle": {"apt": "gradle", "brew": "gradle", "choco": "gradle", "winget": "
Gradle.Gradle"},
    "aws_cli": {"apt": "awscli", "brew": "awscli", "choco": "awscli", "winget":
"Amazon.AWSCLI"},
    "azure_cli": {"apt": "azure-cli", "brew": "azure-cli", "choco": "azure-cli",
 "winget": "Microsoft.AzureCLI"},
    "gcloud_sdk": {"apt": "google-cloud-sdk", "brew": "google-cloud-sdk", "choco
": "gcloudsdk", "snap": "google-cloud-cli"},
    # Add more mappings
}
...
def _run_pm_command(command_parts, timeout=15):
    ...
    try:
        ...
        # For Linux commands that might need sudo, it's better to inform the use
r to run the app with sudo
        # or provide copyable commands. Auto-sudo is risky.
        # However, for read-only commands like search/list, sudo is usually not
needed.
        process = subprocess.Popen(command_parts, stdout=subprocess.PIPE, stderr
=subprocess.PIPE, text=True, encoding='utf-8', errors='replace')
        stdout, stderr = process.communicate(timeout=timeout)
        if process.returncode != 0:
            logger.warning(f"PM Command '{' '.join(command_parts)}' failed with
code {process.returncode}. Stderr: {stderr.strip()}")
...
def detect_package_managers():
    ...
    for pm_id, pm_config in PACKAGE_MANAGERS.items():
        if current_os in pm_config["os"]:
            ...
            if exe_path:
                detected_pms[pm_id] = {"name": pm_config["name"], "path": exe_pa
th}
                logger.info(f"Detected package manager: {pm_config['name']} at {
exe_path}")
    return detected_pms
...
def parse_version_from_output(output, pm_id, package_name_in_pm):
    ...
    logger.debug(f"Parsing version for '{package_name_in_pm}' from {pm_id} outpu
t:\n{output[:500]}")
    ...
    if pm_id == "winget": # Example: Name Id Version Available Source
                         #        ----------------------------------------------
-------------
                         #        Python 3 Python.Python.3 3.9.7 3.9.150.0 winge
t
        ...
        for line in lines:
            if "Id" in line and "Version" in line: # Header
                header_found = True
                header_parts = [p.strip() for p in re.split(r'\s{2,}', line)] #
Split by 2+ spaces
                try:
                    ...
                    if version_col == -1: # Check if Version column was found
                        logger.warning(f"Could not find 'Version' column in wing
et output header: {line}")
                        return None
                except ValueError: # For id_col = header_parts.index("Id")
                    logger.warning(f"Could not find 'Id' column in winget output
 header: {line}")
                    return None
                continue
            ...
            if header_found and id_col != -1 and version_col != -1:
                ...
                if len(parts) > max(id_col, version_col) and parts[id_col] == pa
ckage_name_in_pm:
                    version = parts[version_col]
                    break
        if not version: logger.debug(f"Winget: No exact match for ID '{package_n
ame_in_pm}' or version not found.")


    elif pm_id == "choco": # choco search <pkg> --exact
                           # <package_name_in_pm>|<version>
        match = re.search(rf"^{re.escape(package_name_in_pm)}\|([0-9a-zA-Z\.\-]+
)", output, re.MULTILINE)
        if match:
        ...
        match = re.search(rf"{re.escape(package_name_in_pm)}\s+\(([0-9a-zA-Z\.\-
]+)\)", output)
        if match:
            version = match.group(1)
        ...
        match = re.search(rf"{re.escape(package_name_in_pm)}: stable\s+([0-9a-zA
-Z\.\-]+)", output)
        if not match: # Sometimes it's just <formula>: <version> or <formula> <v
ersion>
             # Try format "package_name: version" first
             match = re.search(rf"^{re.escape(package_name_in_pm)}:\s+([0-9]+\.[
0-9]+(?:[\.\_][0-9a-zA-Z]+)*)", output, re.MULTILINE)
        if not match: # Then try format "package_name version"
             match = re.search(rf"^{re.escape(package_name_in_pm)}\s+([0-9]+\.[0
-9]+(?:[\.\_][0-9a-zA-Z]+)*)", output, re.MULTILINE)
        if match:
            version = match.group(1)
    ...
    elif pm_id == "dnf" or pm_id == "yum": # dnf info <package_name> or yum info
 <package_name>
                                          # Version      : <version>
        ...
        match = re.search(r"latest/stable:\s*([0-9a-zA-Z\.\-]+)\s*", output) # T
racks a channel
    ...
    if version:
        logger.info(f"Parsed version for '{package_name_in_pm}' via {pm_id}: {ve
rsion}")
    else:
        logger.warning(f"Could not parse version for '{package_name_in_pm}' via
{pm_id} from output.")
...
def get_latest_version_and_update_command(tool_id, tool_name, installed_version,
 preferred_pms_os_specific):
    """
    Queries preferred package managers for the latest version of a given tool an
d its update command.
...
for the current OS.
    ...
    Returns:
        dict or None: A dictionary with 'latest_version', 'package_manager', 'up
date_command',
                      'package_name_in_pm' if an update is found, else None.
    """
    logger.debug(f"Checking updates for {tool_name} (ID: {tool_id}, Installed: {
installed_version}) via {preferred_pms_os_specific}")
    ...
    for pm_id in preferred_pms_os_specific:
        if pm_id not in PACKAGE_MANAGERS or pm_id not in detected_system_pms:
            logger.debug(f"Skipping {pm_id}: not configured or not detected on s
ystem.")
            continue
        ...
        if not package_name_in_pm:
            logger.debug(f"No package mapping for tool '{tool_id}' in package ma
nager '{pm_id}'.")
            continue

        logger.info(f"Querying {pm_config['name']} for {tool_name} (package: {pa
ckage_name_in_pm})...")
        ...
        search_cmd = [part.replace("{package_name}", package_name_in_pm).replace
("{package_id}", package_name_in_pm) for part in search_cmd_template]
        ...
        if stdout:
            latest_version = parse_version_from_output(stdout, pm_id, package_na
me_in_pm)
            if latest_version:
                update_command_template = pm_config.get("update_cmd_template")
                update_cmd_str = ""
                if update_command_template:
                    # Construct the update command string, ensuring the PM execu
table is present
                    base_cmd_parts = [part.replace("{package_name}", package_nam
e_in_pm).replace("{package_id}", package_name_in_pm) for part in update_command_
template]
                    if not any(pm_config["detection_exe"] in part for part in ba
se_cmd_parts):
                        update_cmd_str = f"{pm_config['detection_exe']} {' '.joi
n(base_cmd_parts)}"
                    else:
                        update_cmd_str = ' '.join(base_cmd_parts)


                # Basic version comparison (can be improved with packaging libra
ry for semantic versioning)
                is_update_available = False
                if installed_version and latest_version and installed_version !=
 "Unknown":
                    try:
                        from packaging.version import parse as parse_version # K
eep import here for clarity
                        if parse_version(latest_version) > parse_version(install
ed_version):
                            is_update_available = True
                    except ImportError:
                        logger.warning("The 'packaging' library is not installed
. Falling back to string comparison for versions. Run 'pip install packaging' fo
r more accurate version handling.")
                        if latest_version > installed_version: # Lexicographical
                            is_update_available = True
                    except Exception as e_parse: # Handles packaging.version.Inv
alidVersion and other parsing errors
                        logger.warning(f"Could not parse/compare versions '{inst
alled_version}' and '{latest_version}' using packaging.version: {e_parse}. Falli
ng back to string comparison.")
                        if latest_version > installed_version: # Lexicographical
                            is_update_available = True
            ...
            logger.debug(f"No output from {pm_id} for package {package_name_in_p
m}.")

    logger.info(f"No update information found for {tool_name} via preferred pack
age managers.")
    return None
...
")
    # First, try to get installed Git version (mocked or use scan_logic if avail
able)
    ...
    update_info = get_latest_version_and_update_command(git_tool_id, git_tool_na
me, installed_git_version, prefs)
    ...
    if update_info:
        ...
        print("\nNo update information found for Git via preferred package manag
ers.")

    # Example for a tool that might not be installed or has complex naming
    print(f"\nTesting update check for 'NonExistentTool'...")
    update_info_nonexistent = get_latest_version_and_update_command("nonexistent
tool", "NonExistentTool", "1.0", prefs)
    if update_info_nonexistent:
        print(f"  Found something for NonExistentTool (unexpected): {update_info
_nonexistent}")
    else:
