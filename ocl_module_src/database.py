...
def _update_profile_last_modified(conn, profile_id: int):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE profiles SET last_modified_date = ? WHERE id = ?",
            (now_iso, profile_id),
        )
...
def list_all_profiles() -> list[dict]:
    ...
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, name, description, last_modified_date FROM profiles ORDER BY last_modified_date DESC"
            )
...
def update_profile(
    profile_id: int, name: str | None = None, description: str | None = None
) -> bool:
...
def get_settings_for_profile(profile_id: int) -> list[dict]:
    ...
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM settings WHERE profile_id = ? ORDER BY category, setting_name",
                (profile_id,),
            )
            rows = cursor.fetchall()
            return [dict(row) for row in rows]
