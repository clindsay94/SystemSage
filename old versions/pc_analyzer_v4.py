# Python Software Inventory & Path Validator v4
...
# V4: Added Registry Key Path to output.
            ...
            with winreg.OpenKey(hkey_root, path_suffix) as uninstall_key:
                for i in range(winreg.QueryInfoKey(uninstall_key)[0]):
                    ...
                    try:
                        ...
                        # V4: Construct the full registry key path
                        ...
                        with winreg.OpenKey(uninstall_key, subkey_name) as app_key:
                            app_details = {
                                ...
                                'RegistryKeyPath': full_reg_key_path # V4: Added
                            }
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            for app in software_list:
                if app.get('Category') == "Application":
                    ...
                    reg_key = str(app.get('RegistryKeyPath', 'N/A')).replace('|', '\|') # V4: Added
                    f.write(f"| {name} | {version} | {publisher} | {location} | {status} | {hive} | {reg_key} |\n") # V4: Added reg_key
            if app_count == 0:
                f.write("| No primary applications found. | | | | | | |\n") # V4: Adjusted for new column

            if include_components:
                f.write("\n## Components & Drivers\n")
                f.write(header.replace("Application Name", "Component Name")) # Adjust header for this section
                f.write(separator)
                comp_count = 0
                for app in software_list:
...
if __name__ == "__main__":
    print("Software Inventory and Path Validator v4")
    print("=" * 40)
    ...
    if console_apps_to_display:
        # V4: Adjusted console output width and added Registry Key Path
        print(f"\nFound {len(console_apps_to_display)} applications (defaulting to no components/drivers for console):\n")
        # Increased total width to accommodate the new column. Adjust column widths as needed.
        # Example: Name 40, Version 12, Publisher 25, Path 40, Status 20, Source 12, RegKey 60 = 209 + separators
        print(f"{'Application Name':<40} | {'Version':<12} | {'Publisher':<25} | {'Install Path':<40} | {'Status':<20} | {'Source':<12} | {'Registry Key Path':<60}")
        print("-" * 220) # Adjusted separator length
        ...
        for app in console_apps_to_display:
            ...
            reg_key = str(app.get('RegistryKeyPath', 'N/A')) # V4: Added
