...
def update_existing_profile(
    ...
    try:
        ...
        if name is not None or description is not None:
            ...
            attempted_any_update = (
                True  # update_profile itself returns bool, but we track attempt
            )
        ...
        if settings_to_update:
            ...
            for setting_update in settings_to_update:
                database.update_setting_value(
                    setting_update["id"], setting_update["setting_value"]
                )
        ...
        return (
            attempted_any_update  # Or False if strict success for all parts is needed
        )
...
if __name__ == "__main__":
    ...
    new_profile_id1 = create_new_profile(
        "My Awesome OC",
        "Daily driver overclock for Ryzen 9",
        profile1_settings,
        profile1_logs,
    )
    if new_profile_id1:
    ...
    new_profile_id2 = create_new_profile(
        "Gaming Profile", "Max performance for gaming", initial_logs=["Baseline test."]
    )
    if new_profile_id2:
    ...
    if new_profile_id1:
        ...
        if details:
            ...
            for setting in details.get("settings", []):
                print(
                    f"    - {setting.get('category')}/{setting.get('setting_name')}: {setting.get('setting_value')}"
                )
    ...
    if new_profile_id1:
        ...
        profile1_settings_to_add = [
            {
                "category": "FanControl",
                "setting_name": "CPU_Fan_Curve",
                "setting_value": "Aggressive",
                "value_type": "str",
            }
        ]
        ...
        if (
            profile1_details_before_update
            and profile1_details_before_update["settings"]
        ):
            setting_to_update_id = profile1_details_before_update["settings"][0][
                "id"
            ]  # Get ID of first setting

        profile1_settings_to_update = []
        if setting_to_update_id:
            profile1_settings_to_update.append(
                {"id": setting_to_update_id, "setting_value": "1.28"}
            )  # Update CoreVoltage
    ...
    if new_profile_id2:
        ...
        log_id = add_log_to_profile(
            new_profile_id2, "Ran FurMark stress test - stable."
        )
        if log_id:
    ...
    if new_profile_id2 and get_profile_details(
        new_profile_id2
        ...
        if profile2_details and profile2_details["settings"]:
            ...
            update_success = update_existing_profile(
                new_profile_id2, setting_ids_to_delete=[setting_to_delete_id]
            )
            print(
                f"Update (delete setting) for profile {new_profile_id2} attempted: {update_success}"
            )
            details_after_setting_delete = get_profile_details(new_profile_id2)
            if details_after_setting_delete:
                print(
                    f"  Settings now: {len(details_after_setting_delete.get('settings', []))} items"
                )
