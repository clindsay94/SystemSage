...
PACKAGE_MANAGERS = {
    "winget": {
        ...
        "search_latest_cmd_template": [
            ...
            "--exact",
            "{package_name}",
        ],  # winget search --exact <package_id>
        "update_cmd_template": [
            "winget",
            "upgrade",
            "--exact",
            "{package_name}",
        ],  # winget upgrade --exact <package_id>
        "os": ["Windows"],
    },
    "choco": {
        "name": "Chocolatey",
    ...
    "scoop": {
        ...
        "search_latest_cmd_template": [
            ...
            "search",
            "{package_name}",
        ],  # scoop search <package_name> (exactness varies)
    ...
    "pacman": {
        ...
        "search_latest_cmd_template": [
            ...
            "-Si",
            "{package_name}",
        ],  # pacman -Si <package_name> (sync DB info)
        "update_cmd_template": [
            "sudo",
            "pacman",
            "-Syu",
            "{package_name}",
        ],  # This updates all then installs, or just -S for specific
        "os": ["Linux"],
    },
    "snap": {
        "name": "Snap",
    ...
    "flatpak": {
        ...
        "search_latest_cmd_template": [
            ...
            "flathub",
            "{package_id}",
        ],  # Assumes flathub, package_id is like org.example.App
        "update_cmd_template": [
            "flatpak",
            "update",
            "-y",
...
def _run_pm_command(command_parts, timeout=15):
    ...
    try:
        ...
        if process.returncode != 0:
            logger.warning(
                f"PM Command '{' '.join(command_parts)}' failed with code {process.returncode}. Stderr: {stderr.strip()}"
            )
    ...
    for pm_id, pm_config in PACKAGE_MANAGERS.items():
        if current_os in pm_config["os"]:
            ...
            if exe_path:
                ...
                logger.info(
                    f"Detected package manager: {pm_config['name']} at {exe_path}"
                )
    return detected_pms
...
def parse_version_from_output(output, pm_id, package_name_in_pm):
    ...
    logger.debug(
        f"Parsing version for '{package_name_in_pm}' from {pm_id} output:\n{output[:500]}"
    )
    ...
    if pm_id == "winget":  # Example: Name Id Version Available Source
        ...
        for line in lines:
            if "Id" in line and "Version" in line:  # Header
                header_found = True
                header_parts = [
                    p.strip() for p in re.split(r"\s{2,}", line)
                ]  # Split by 2+ spaces
                try:
                    ...
                    if version_col == -1:  # Check if Version column was found
                        logger.warning(
                            f"Could not find 'Version' column in winget output header: {line}"
                        )
                        return None
                except ValueError:  # For id_col = header_parts.index("Id")
                    logger.warning(
                        f"Could not find 'Id' column in winget output header: {line}"
                    )
                    return None
                continue
            ...
            if header_found and id_col != -1 and version_col != -1:
                ...
                if (
                    len(parts) > max(id_col, version_col)
                    and parts[id_col] == package_name_in_pm
                ):
                    version = parts[version_col]
                    break
        if not version:
            logger.debug(
                f"Winget: No exact match for ID '{package_name_in_pm}' or version not found."
            )

    elif pm_id == "choco":  # choco search <pkg> --exact
        # <package_name_in_pm>|<version>
        match = re.search(
            rf"^{re.escape(package_name_in_pm)}\|([0-9a-zA-Z\.\-]+)",
            output,
            re.MULTILINE,
        )
        if match:
        ...
        match = re.search(
            rf"{re.escape(package_name_in_pm)}\s+\(([0-9a-zA-Z\.\-]+)\)", output
        )
        if match:
            version = match.group(1)
        ...
        match = re.search(
            rf"{re.escape(package_name_in_pm)}: stable\s+([0-9a-zA-Z\.\-]+)", output
        )
        if not match:  # Sometimes it's just <formula>: <version> or <formula> <version>
            # Try format "package_name: version" first
            match = re.search(
                rf"^{re.escape(package_name_in_pm)}:\s+([0-9]+\.[0-9]+(?:[\.\_][0-9a-zA-Z]+)*)",
                output,
                re.MULTILINE,
            )
        ...
        pm_id == "dnf" or pm_id == "yum"
    ):  # dnf info <package_name> or yum info <package_name>
        # Version      : <version>
        ...
        match = re.search(
            r"latest/stable:\s*([0-9a-zA-Z\.\-]+)\s*", output
        )  # Tracks a channel
    ...
    if version:
        ...
        logger.warning(
            f"Could not parse version for '{package_name_in_pm}' via {pm_id} from output."
        )

    return version


def get_latest_version_and_update_command(
    tool_id, tool_name, installed_version, preferred_pms_os_specific
):
    """
    ...
    """
    logger.debug(
        f"Checking updates for {tool_name} (ID: {tool_id}, Installed: {installed_version}) via {preferred_pms_os_specific}"
    )
    ...
    for pm_id in preferred_pms_os_specific:
        ...
        if not package_name_in_pm:
            logger.debug(
                f"No package mapping for tool '{tool_id}' in package manager '{pm_id}'."
            )
            continue

        logger.info(
            f"Querying {pm_config['name']} for {tool_name} (package: {package_name_in_pm})..."
        )
        ...
        search_cmd = [
            part.replace("{package_name}", package_name_in_pm).replace(
                "{package_id}", package_name_in_pm
            )
            for part in search_cmd_template
        ]
        ...
        if stdout:
            latest_version = parse_version_from_output(
                stdout, pm_id, package_name_in_pm
            )
            if latest_version:
                ...
                if update_command_template:
                    ...
                    base_cmd_parts = [
                        ...
                        for part in update_command_template
                    ]
                    if not any(
                        pm_config["detection_exe"] in part for part in base_cmd_parts
                    ):
                        update_cmd_str = (
                            f"{pm_config['detection_exe']} {' '.join(base_cmd_parts)}"
                        )
                    else:
                ...
                if (
                    ...
                    try:
                        ...
                        logger.warning(
                            "The 'packaging' library is not installed. Falling back to string comparison for versions. Run 'pip install packaging' for more accurate version handling."
                        )
                        ...
                        logger.warning(
                            f"Could not parse/compare versions '{installed_version}' and '{latest_version}' using packaging.version: {e_parse}. Falling back to string comparison."
                        )
                        if latest_version > installed_version:  # Lexicographical
                            is_update_available = True
    ...
    logger.info(
        f"No update information found for {tool_name} via preferred package managers."
    )
    return None
...
if __name__ == "__main__":
    ...
    update_info = get_latest_version_and_update_command(
        git_tool_id, git_tool_name, installed_git_version, prefs
    )
    ...
    update_info_nonexistent = get_latest_version_and_update_command(
        "nonexistent_tool", "NonExistentTool", "1.0", prefs
    )
    if update_info_nonexistent:
        print(
            f"  Found something for NonExistentTool (unexpected): {update_info_nonexistent}"
        )
    else:
