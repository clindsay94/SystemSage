...
class TestJsonOutput(unittest.TestCase):
    ...
    sample_dev_comp = [
        DetectedComponent(
            id="comp1", name="ToolA", version="1.1", path="/usr/bin/toola"
        )
    ]
    sample_dev_env = [
        EnvironmentVariableInfo(name="PATH", value="/usr/bin:/bin", scope="Test")
    ]
    ...
    def run_json_test(self, data, mock_log_info, mock_open_file, mock_makedirs):
        ...
        if (
            not data.get("inventory")
            and not data.get("dev_comp")
            and not data.get("dev_env")
            and not data.get("dev_issues")
        ):
            output_to_json_combined(
                None, None, None, None, output_dir, "test_report.json"
            )
            ...
            if mock_log_info.call_args_list:  # Check if logging.info was called
                self.assertIn(
                    "No data to save to JSON report",
                    mock_log_info.call_args_list[0][0][0],
                )
    ...
    def test_json_full_data(self):
        ...
        self.assertEqual(
            result["devEnvAudit"]["detectedComponents"][0]["name"], "ToolA"
        )
        self.assertEqual(
            result["devEnvAudit"]["environmentVariables"][0]["name"], "PATH"
        )
        self.assertEqual(
            result["devEnvAudit"]["identifiedIssues"][0]["description"], "Test issue"
        )
...
class TestMarkdownOutput(unittest.TestCase):
    ...
    sample_dev_comp = [
        DetectedComponent(
            ...
            version="1.1",
            path="/path",
            executable_path="/path/toola",
        )
    ]
    sample_dev_env = [
        EnvironmentVariableInfo(name="TEST_VAR", value="test_value", scope="User")
    ]
    sample_dev_issues = [
        ScanIssue(
            severity="High",
