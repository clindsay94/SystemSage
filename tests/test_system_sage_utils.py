...
from SystemSageV1_2 import format_size, is_likely_component, load_json_config, \
                           DEFAULT_COMPONENT_KEYWORDS, DEFAULT_LAUNCHER_HINTS, \
                           COMPONENT_KEYWORDS, LAUNCHER_HINTS # Import live ones too for some tests
...
class TestFormatSize(unittest.TestCase):
    def test_format_size_various(self):
        ...
        self.assertEqual(format_size(0, False), "Not Calculated") # Test 'Not Calculated'
        self.assertEqual(format_size(100, False), "Not Calculated") # Test 'Not Calculated' with size
...
class TestIsLikelyComponent(unittest.TestCase):
    ...
    # This assumes COMPONENT_KEYWORDS is loaded correctly in SystemSageV1_2.py
    # If SystemSageV1_2.py is not run before tests, COMPONENT_KEYWORDS might be the default one.
    # For more robust testing, one might need to mock the loading of COMPONENT_KEYWORDS
    # or ensure SystemSageV1_2.COMPONENT_KEYWORDS is explicitly set/reloaded.
    def test_keywords(self):
        # Test with actual COMPONENT_KEYWORDS if they are loaded from file,
        # otherwise it will use DEFAULT_COMPONENT_KEYWORDS if file loading failed in SystemSageV1_2.py
        self.assertTrue(is_likely_component("Some SDK", "Anycorp")) # "sdk" is a keyword
        self.assertTrue(is_likely_component("Driver Update", "Anycorp")) # "driver" is a keyword
        self.assertTrue(is_likely_component("Visual C++ Redistributable", "Microsoft")) # "visual c++" and "redistributable"
        self.assertFalse(is_likely_component("My Application", "MyCompany"))

    def test_heuristics(self):
        self.assertTrue(is_likely_component("{GUID-LIKE-STRING}", "Anycorp"))
        self.assertTrue(is_likely_component("KB123456", "Microsoft"))
...
class TestLoadJsonConfig(unittest.TestCase):
    ...
    def test_invalid_json(self, mock_file_open, mock_path_exists):
        ...
        with patch('json.load', side_effect=json.JSONDecodeError("err", "doc", 0)) as mock_json_load:
